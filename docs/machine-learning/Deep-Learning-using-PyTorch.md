...
강좌를 들은 후, 많은 수의 데이터 $x$, $y$가 주어지고 이 데이터를 나타내기 적합한 계수 $w$, $b$를 찾는 것이라고 생각하게 되었습니다.
...
수학자가 현상/데이터를 분석하여 적절한 수식을 도출하거나, 개발자가 엔지니어링 접근법을 통해 적절한 수식을 찾고, 이를 프로그래밍에서 사용하는 것이 일반적이었는데요.
데이터 분석가가 데이터를 분석 및 정제하고 입/출력 데이터와 대략적인 수식(가설, hypothesis)을 입력하면 학습을 통해 적절한 수식 도출하여 프로그램에서 활용하는 방법도 일반화된 것이지요.
...
`2*(hx-y[i])*x[i]`을 도출할 수 있습니다. 

(합성함수 미분을 하고..... Forward propagation과 관련있고.... 하나하나 설명하기 어려워 우선 넘어가겠습니다 :sweat_smile:)
